...
class FirebaseDatabaseSender {
  ...
  final DatabaseReference _databaseReference =
      FirebaseDatabase.instanceFor(app: Firebase.app(), databaseURL: firebaseDatabaseUrl).ref();
  ...
  void sendDioExceptionLog({required DioException dioException}) async {
    ...
    String path = dioException.requestOptions.baseUrl + dioException.requestOptions.path;
    ...
    if (dioException.type == DioExceptionType.badResponse) {
      ...
      final String errorCode = dioException.response?.statusCode?.toString() ?? "-";
  ...
  void sendErrorMessageLog({required String message, required String stackTrace}) async {
    String monthYear = DateTime.now().convert(format: "yyyy-MM");
    String date = DateTime.now().convert(format: "dd MMMM yyyy");
    String time = DateTime.now().convert(format: "HH:mm:ss");
    String token = _userPreference.getToken() ?? "-";
